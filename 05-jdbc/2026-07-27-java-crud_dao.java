...
//배열출력
	...
	public ArrayList<SnackDto> getSnackAll() {
	ArrayList<SnackDto> arr = new ArrayList<>();
	String sql = "select pcode,pname,company,makedate,price\r\n"
			+ "from snack_홍길동";
	...
	try {
		...
		System.out.println("메소드오류");
		System.out.println("select pcode,pname,company,makedate,price\r\n"
				+ "from snack_홍길동");
		e.printStackTrace();
	}finally {
		DBConnection.closeDB(con, ps, rs);
	...
	public String getName2(String searchName) {
		String result = "";
		String sql = "select pname\r\n"
				+ "from snack_홍길동\r\n"
				+ "where pcode = '"+searchName+"'";
	...
	//인서트메소드
	public int snackSave(SnackDto saveDto) {
		int result = 0;
		String sql = "insert into snack_권구봉\r\n"
				+ "(pcode,pname,company,makedate,price)\r\n"
				+ "values\r\n"
				+ "('"+saveDto.getPcode()+"','"+saveDto.getPname()+"','"+saveDto.getCompany()+"','"+saveDto.getMakedate()+"','"+saveDto.getPrice()+"')";
	...
	public int snackUpdateName(SnackDto a) {
		int result = 0;
		String sql = "update snack_권구봉\r\n"
				+ "set pname = '"+a.getPname()+"'\r\n"
				+ "where pcode = '"+a.getPcode()+"'";
		try {
			con = DBConnection.getConnection();
			ps = con.prepareStatement(sql);
			result = ps.executeUpdate();
			...
			System.out.println("스낵디티오메소드오류");
			System.out.println("update snack_권구봉\r\n"
				+ "set pname = '"+a.getPname()+"'\r\n"
				+ "where pcode = '"+a.getPcode()+"'");
			e.printStackTrace();
		}finally {
	...
	public int snackUpdateCompany(SnackDto a) {
		int result = 0;
		String sql = "update snack_권구봉\r\n"
				+ "set company = '"+a.getCompany()+"'\r\n"
				+ "where pcode = '"+a.getPcode()+"'";
	...
	public int snackUpdatePrice(SnackDto a) {
		int result = 0;
		String sql = "update snack_권구봉\r\n"
...
//업뎃
	public int snackUpdateDate(SnackDto a) {
		int result = 0;
		String sql = "update snack_권구봉\r\n"
				+ "set makedate = '"+a.getMakedate()+"'\r\n"
				+ "where pcode = '"+a.getPcode()+"'";
		try {
			con = DBConnection.getConnection();
			ps = con.prepareStatement(sql);
